<div class="space-y-6">
    ...
    <div
        ...
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400">Controla los roles y permisos de los miembros del equipo.
            </p>
        </div>
        @if (auth()->user()->role === 'admin')
            <button wire:click="openModal"
                class="inline-flex items-center gap-2 bg-black text-white font-bold text-xs px-6 py-3 cursor-pointer rounded-sm hover:opacity-90 transition-all uppercase tracking-widest shadow-lg">
                ...
                @foreach ($users as $user)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors group">
                        <td class="px-6 py-4">
                            <div class="flex items-center">
                                <img class="h-8 w-8 rounded-full"
                                    ...
                                    alt="">
                                <div class="ml-4">
                                    <div class="text-sm font-bold text-gray-900 dark:text-white">{{ $user->name }}
                                    </div>
                                    <div class="text-[10px] text-gray-500 font-medium tracking-tight">
                                        {{ $user->email }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span
                                class="inline-flex items-center px-2 py-0.5 text-[10px] font-bold rounded-sm uppercase tracking-wider {{ $user->role === 'admin' ? 'bg-black text-white dark:bg-white dark:text-black' : 'bg-gray-100 text-gray-600' }}">
                                {{ $user->role }}
                            </span>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex flex-wrap gap-1">
                                @forelse($user->permissions ?? [] as $perm)
                                    <span
                                        class="px-1 py-0.5 bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 text-[8px] font-bold uppercase rounded-sm text-gray-500">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-xs font-bold">
                            @if (auth()->user()->role === 'admin')
                                <button wire:click="edit({{ $user->id }})"
                                    class="text-brand-600 hover:text-brand-900">EDITAR</button>
                            @else
    ...
    @if ($showModal)
        <div class="fixed inset-0 z-[100] overflow-y-auto">
            <div class="flex items-center justify-center min-h-screen px-4">
                ...
                <div
                    class="relative bg-white dark:bg-gray-900 w-full max-w-2xl rounded-sm shadow-2xl border-t-4 border-brand-600 overflow-hidden">
                    <div
                        ...
                        <h3 class="text-xl font-serif font-bold">
                            {{ $userId ? 'Configurar Usuario' : 'Nuevo Miembro de Equipo' }}</h3>
                    ...
                    <div class="p-8 space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" wire:model="name"
                                    class="w-full bg-gray-50 dark:bg-gray-800 border-none rounded-sm text-sm p-3 font-bold">
                                @error('name')
                            ...
                            <div>
                                <label
                                    class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Email</label>
                                <input type="email" wire:model="email"
                                    class="w-full bg-gray-50 dark:bg-gray-800 border-none rounded-sm text-sm p-3">
                                @error('email')
                                    <span class="text-red-500 text-[10px] font-bold">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        @if (!$userId)
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label
                                        class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Contraseña</label>
                                    <input type="password" wire:model="password"
                                        class="w-full bg-gray-50 dark:bg-gray-800 border-none rounded-sm text-sm p-3">
                                </div>
                                <div>
                                    <label
                                        class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Confirmar</label>
                        ...
                        @if ($role !== 'admin')
                            <div class="pt-6 border-t border-gray-100 dark:border-gray-800">
                                <label
                                    class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3">Permisos
                                    de Gestión</label>
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-2">
                                    @foreach ($availablePermissions as $key => $label)
                                        <button wire:click="togglePermission('{{ $key }}')" type="button"
                                            class="px-4 py-2 text-[10px] font-bold border rounded-sm transition-all uppercase tracking-tight {{ in_array($key, $permissions) ? 'bg-brand-50 text-brand-700 border-brand-200' : 'bg-gray-50 text-gray-400 border-gray-100 dark:bg-gray-800 dark:border-gray-700' }}">
